<?php
	...
	function my_post_image_html( $html, $post_id, $post_image_id ) {
		$html = '<a href="'.wp_get_attachment_url(get_post_thumbnail_id($post_id)).'" data-lightbox="lightbox">'.$html.'</a>';
		return $html;
	}
	function get_first_image($post) {
		...
		if (count($aPics[0]) > 0) {
		   for ($i=0; $i < count($aPics[0]); $i++) {
		        ...
		        return '<a href="'.$matches[1].'" data-lightbox="lightbox">'.$aPics[0][$i].'</a>';
		   };
		} else {
